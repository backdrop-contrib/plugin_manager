...
To define that you offer a plugin that modules can implement, you first must implement hook_plugin_manager_plugin_type() to tell the plugin system about your plugin.

```php
/**
 * Implements hook_plugin_manager_plugin_type() to inform plugin_manager about the layout plugin.
 */
function panels_plugin_manager_plugin_type() {
...
If set to TRUE, the results of plugin_manager_get_plugins will be cached in the 'cache' table (by default), thus preventing .inc files from being loaded. plugin_manager_get_plugins looking for a specific plugin will always load the appropriate .inc file.
...
An array of *class identifiers*(i.e. plugin array keys) which a plugin of this type uses to provide classes to the plugin_manager autoloader. For example, if **classes** is set to array('class'), then plugin_manager will search each **$plugin['class']** for a class to autoload. Depending of the plugin structure, a *class identifier* may be either:
...
An optional function callback to use for processing a plugin. This can be used to provide automated settings that must be calculated per-plugin instance (i.e., it is not enough to simply append an array via 'defaults'). The parameters on this callback are: **callback(&$plugin, $info)** where $plugin is a reference to the plugin as processed and $info is the fully processed result of hook_plugin_manager_plugin_api_info().
**extension**
*Defaults to:* **'inc'**
...
To create a plugin, a module only has to execute plugin_manager_get_plugins with the right data:
...
type of the plugin. It is typically best practice to provide some kind of wrapper
function to make this easier. For example, Panels provides the following
...
function panels_get_content_type($content_type) {
  plugin_manager_include('context');
  plugin_manager_include('plugins');
  return plugin_manager_get_plugins('panels', 'content_types', $content_type);
}
...
function panels_get_content_types() {
  plugin_manager_include('context');
  plugin_manager_include('plugins');
  return plugin_manager_get_plugins('panels', 'content_types');
}
```
...
The most likely data (beyond simple printable data) for a plugin to provide is a callback. The plugin system provides a pair of functions to make it easy and consistent for these callbacks to be used. The first is plugin_manager_plugin_get_function, which requires the full $plugin object.

```php
/**
 * Get a function from a plugin, if it exists. If the plugin is not already
 * loaded, try plugin_manager_plugin_load_function() instead.
 *
 * @param $plugin
